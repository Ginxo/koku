...
class ReportParquetProcessorBase:
    ...
    def _generate_create_table_sql(self, partition_map=None):
        ...
        if partition_map:
            ...
            sql += f") WITH(format = 'PARQUET'," f" partitioning=ARRAY[{partition_column_str}])"
        else:
            # Use the default partition columns
            sql += ",source varchar, year varchar, month varchar"
            sql += ") WITH(format = 'PARQUET'," " partitioning=ARRAY['source', 'year', 'month'])"

        LOG.debug("THIS IS THE CREATE TABLE SQL")
        LOG.debug(sql)
    ...
    def sync_hive_partitions(self):
        ...
        "CALL system.add_files(table => 'self._table_name', source_table => '`parquet`.`path/to/table`');"
        LOG.info(sql)
        self._execute_sql(sql, self._schema_name)
